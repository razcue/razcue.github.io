 ...
 *
 * Usage:
  *   node build-resume.cjs [config-name]
  *   node build-resume.cjs 2025-02-10-google-senior-frontend-engineer
  *   node build-resume.cjs default
  *   node build-resume.cjs default short    # Build short resume with highlights
  *   node build-resume.cjs 2025-02-10-google short    # Build short tailored resume
  *
  * Config files:
  *   - resume-build/configs/default.json (base profile, contact, education)
  *   - resume-build/configs/YYYY-MM-DD-{company}-{role}.json (tailored resumes)
  *
  * The script will:
  *   1. Load the config
  ...
  *   7. If "short" param: use experienceHighlights instead of experience
  */
...
/**
 * Parse experience highlights to extract text and company
 */
function parseHighlights(highlights) {
  return highlights.map((hl) => {
    const match = hl.match(/^(.+?)\s*@(.+)$/);
    if (match) {
      return { text: match[1].trim(), company: match[2].trim() };
    }
    return { text: hl, company: '' };
  });
}
 ...
 */
function renderResume(templateSource, config, isShort) {
  const template = Handlebars.compile(templateSource);

  // Parse experience highlights if in short mode
  const parsedHighlights = isShort
    ? parseHighlights(config.experienceHighlights || [])
    : [];
  ...
  const data = {
    ...
    experience: isShort ? [] : (config.experience || []),
 ...
 * Main build function
 */
async function buildResume(configName, positionJsonPath, isShort) {
  console.log(`\n📄 Building resume from config: ${configName}${isShort ? ' (SHORT)' : ''}\n`);
  ...
  try {
    ...
    // Render HTML
    console.log('\n4️⃣  Rendering HTML...');
    const html = renderResume(templateSource, mergedConfig, isShort);
    ...
    const pdfFilename =
      configName === 'default'
        ? (isShort ? 'Rayko_Azcue_Resume.pdf' : 'Rayko_Azcue_CV.pdf')
        : `resume-${configName}${isShort ? '-short' : ''}.pdf`;
...
if (require.main === module) {
  ...
  // Check for 'short' flag anywhere in args
  const isShort = args.includes('short');

  // positionJsonPath is the last arg if it's not 'short'
  const positionJsonPath = args.find((arg, idx) => idx > 0 && arg !== 'short') || null;

  buildResume(configName, positionJsonPath, isShort);
}
